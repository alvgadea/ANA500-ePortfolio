...
Act) to two datasets, each project terminating at a different step (per the assignment instructions). 
Portfolio complete.
...
Each project folder is self-contained: the notebook and the data it reads are in the same folder 
together, so every notebook runs from its own directory without editing any paths. All four 
notebooks have been executed end to end, so the figures and output are visible on GitHub without
running anything.
...
instructor feedback from Micro-Project 2 by recoding marital status to a Married/Not-Married
binary, reducing age to three groups, and correcting the class imbalance with class weights
(recall on the >50K class 0.59 to 0.84). Final model is balanced logistic regression
...
network, an LSTM, and a GRU. The result is that the recurrent networks win on every metric 
(LSTM MAE 10.4, RMSE 18.6, R-squared 0.945, against persistence at 10.9 and 19.9), while the
...
beyond the current reading alone, with (a) models using that history will beat a persistence
...
participation, not mentioned at all in the original hypothesis, carries the single largest
